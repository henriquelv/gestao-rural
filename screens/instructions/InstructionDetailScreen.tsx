...
import { User, MapPin, Clock, FileText, ZoomIn, Play, X, Volume2, Eye, Download, Presentation } from 'lucide-react';
...
export const InstructionDetailScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      if (!id) return;
      const items = await db.getInstructions();
      const found = items.find(i => i.id === id);
      if (found && mounted) setInstruction(found);
    };
    load();
  ...
  return (
    <Layout>
      <Header title="Detalhes da Instrução" targetRoute="/instructions/list" />
      <div className="flex-1 overflow-y-auto p-4 space-y-6 bg-gray-50 pb-24">
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200 space-y-4">
          <div className="flex items-center text-gray-500 font-medium">
            ...
            <div className="flex flex-col"><span className="text-xs uppercase font-bold text-gray-400">Data</span><span className="text-lg text-gray-900">{new Date(instruction.createdAt).toLocaleString('pt-BR')}</span></div>
          </div>
          <div className="flex items-center text-gray-500 font-medium border-t border-gray-100 pt-3">
            ...
            <div className="flex flex-col"><span className="text-xs uppercase font-bold text-gray-400">Setor</span><span className="text-lg text-gray-900">{instruction.sector}</span></div>
          </div>
        </div>

        <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
          <h3 className="flex items-center text-lg font-bold text-gray-800 mb-3"><FileText className="mr-2 text-gray-400" /> {instruction.title}</h3>
          {instruction.description && (
            <p className="text-gray-800 text-lg leading-relaxed whitespace-pre-wrap">{instruction.description}</p>
          )}
        </div>
                  ...
                  {m.type === 'photo' && (
                    <img
                      ...
                      onError={(e) => {
                        ...
                        if (next && el.src !== next && !el.dataset.triedFallback) {

...
export const FarmNormsScreen: React.FC = () => {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (f.size > 3 * 1024 * 1024) {
          notify("Arquivo muito grande! Máximo 3MB.", "error");
          return;
      }
  ...
  const handleSave = async () => {
      ...
      if(!responsible) { notify("Defina o responsável.", "error"); return; }
      if(!docFile) { notify("Anexe um documento PDF ou DOC.", "error"); return; }
      ...
      try {
        ...
        notify("Norma salva com sucesso!", "success");
        navigate('/norms/list');
      } catch (e) {
        console.error(e);
        notify("Erro ao salvar. Tente um arquivo menor.", "error");
      } finally {
        setIsSaving(false);
      }
  };
  ...
  return (
    <Layout>
      <Header title="Nova Norma" targetRoute="/norms" />
      ...
      <div className="flex-1 bg-white p-6 space-y-6 overflow-y-auto pb-10">
        ...
        <div>
            ...
            <div className="relative">
                <input value={title} onChange={e => setTitle(e.target.value)} className="w-full p-4 pr-12 border-2 border-gray-200 rounded-xl font-bold text-gray-800 outline-none focus:border-blue-500" placeholder="Digite o título..." />
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            <FieldLabel label="Setor Relacionado" />
            <div className="grid grid-cols-2 gap-2">
                {sectors.map(s => (
                    <button key={s} onClick={() => setSector(s)} className={`p-3 rounded-lg border-2 text-sm font-bold transition-all ${sector === s ? 'bg-blue-600 border-blue-600 text-white shadow-md' : 'bg-gray-50 border-gray-100 text-gray-600 hover:bg-gray-100'}`}>
                        {s}
                    </button>
                ))}
            </div>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 transition-colors hover:border-blue-300">
            <FieldLabel label="Arquivo (PDF, DOC, PPT)" helpText="Máximo 3MB" />
            ...
            {!docFile && (
                <div className="grid grid-cols-1">
                    <label className={`w-full h-32 bg-orange-50 border-2 border-dashed border-orange-300 rounded-xl flex flex-col items-center justify-center active:scale-95 transition-transform shadow-sm cursor-pointer hover:bg-orange-100 group`}>
                        ...
                        <span className="text-xs font-black text-orange-800 text-center leading-tight uppercase">Clique para anexar<br/><span className="text-[10px] font-medium opacity-70">PDF, Word ou PowerPoint</span></span>
                        <input type="file" accept=".pdf,.doc,.docx,.ppt,.pptx" className="hidden" onChange={handleUpload} />
                    </label>
                </div>
            )}
                ...
                <div className="flex flex-col gap-2 mt-2">
                    <div className="flex items-center p-3 bg-green-50 border border-green-200 rounded-lg">
                        ...
                        <div className="flex-1 overflow-hidden">
                            ...
                            <span className="text-[10px] text-green-700 font-bold uppercase flex items-center gap-1"><CheckCircle size={10}/> Pronto para enviar</span>
                        </div>
        ...
        <button onClick={handleSave} disabled={isSaving} className="w-full bg-green-600 active:bg-green-700 text-white py-4 rounded-xl font-black text-xl shadow-lg mt-4 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
            {isSaving ? <><Clock className="animate-spin"/> SALVANDO...</> : <><Save /> SALVAR NORMA</>}
        </button>
      </div>
    </Layout>
  );

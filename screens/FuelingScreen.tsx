...
export const FuelingScreen:React.FC=()=>{
  ...
  const load=async()=>{
    try{const [fuelings,staff,cars,posts]=await Promise.all([db.getFuelings(),db.getEmployees(),db.getFuelVehicles(),db.getFuelStations()]);setRows(fuelings);setVehicles(cars.sort((a,b)=>a.name.localeCompare(b.name,'pt-BR')));setStations(posts.sort((a,b)=>a.name.localeCompare(b.name,'pt-BR')));setEmployees(staff.filter(item=>!item.status||item.status==='active'));}
  ...
  const filtered=useMemo(()=>rows.filter(item=>!month||item.date.startsWith(month)).filter(item=>`${item.vehicle} ${item.vehiclePlate||''} ${item.driverName} ${item.station||''} ${fuelLabel(item.fuelType)}`.toLocaleLowerCase('pt-BR').includes(search.trim().toLocaleLowerCase('pt-BR'))).sort((a,b)=>`${b.date} ${b.time}`.localeCompare(`${a.date} ${a.time}`)),[month,rows,search]);
  ...
  const edit=(item:Fueling)=>{setEditingId(item.id);setForm({vehicle:item.vehicle,vehicleId:item.vehicleId||'',vehiclePlate:item.vehiclePlate||'',stationId:item.stationId||'',date:item.date,time:item.time,odometer:String(item.odometer).replace('.',','),fuelType:item.fuelType,price:formatAutomaticCents(String(Math.round(item.pricePerLiter*100))),total:formatAutomaticCents(String(Math.round(item.totalValue*100))),liters:decimal(item.liters,3),fullTank:item.fullTank,station:item.station||'',driverId:item.driverId||item.employee_id||'',notes:item.notes||''});setMoreOpen(Boolean(item.notes));setView('form');};

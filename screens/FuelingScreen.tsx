  ...
  return <Layout className="bg-[#f4f0e7]"><Header title={historyView?'Histórico de abastecimentos':editingId?'Editar abastecimento':'Novo abastecimento'} targetRoute="/fuelings" />
    <main className="min-h-0 flex-1 touch-pan-y overflow-y-auto overscroll-contain pb-[calc(2rem+env(safe-area-inset-bottom))] [-webkit-overflow-scrolling:touch]">
      ...
      {!historyView?<form onSubmit={save} className="space-y-4 px-4 pt-4 md:grid md:grid-cols-2 md:items-start md:gap-4 md:space-y-0 md:px-6 md:pt-6 xl:grid-cols-3">
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 rounded-2xl border border-[#d8d0c2] bg-white p-3 md:grid-cols-2 xl:grid-cols-3">
          <div className="relative md:col-span-2 xl:col-span-2"><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[#718078]" size={17}/><input type="search" value={search} onChange={event=>setSearch(event.target.value)} placeholder="Buscar veículo, placa, posto ou motorista" className="min-h-12 w-full rounded-xl border-2 border-[#d8d0c2] pl-10 pr-3 text-sm font-bold text-[#173f32]"/></div>
          ...
          {isAdmin&&<div><label className="mb-1.5 block text-[9px] font-black uppercase tracking-wide text-[#718078]">Técnico</label><SearchableSelect value={employeeFilter} options={employeeFilterOptions} onChange={setEmployeeFilter} placeholder="Todos os técnicos" searchPlaceholder="Buscar técnico"/></div>}
          ...
          {hasFilters&&<button type="button" onClick={clearFilters} className="flex min-h-11 w-full items-center justify-center gap-2 self-end rounded-xl bg-[#eef2e9] text-xs font-black text-[#315f45]"><FilterX size={16}/>Limpar filtros</button>}
        </div>

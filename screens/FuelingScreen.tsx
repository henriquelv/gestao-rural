...
import { CalendarDays, CarFront, Check, ChevronDown, Download, Droplets, Edit3, FilterX, Fuel, Gauge, Loader2, MapPin, Plus, Search, Trash2, UserRound } from 'lucide-react';
...
export const FuelingScreen:React.FC=()=>{
  ...
  const filtered=useMemo(()=>rows
    ...
    .sort((a,b)=>`${b.date} ${b.time}`.localeCompare(`${a.date} ${a.time}`)),[employeeFilter,fuelFilter,month,rows,search,stationFilter,vehicleFilter]);
  ...
  const hasFilters=Boolean(search||month||employeeFilter||vehicleFilter||stationFilter||fuelFilter);
  const clearFilters=()=>{setSearch('');setMonth('');setEmployeeFilter('');setVehicleFilter('');setStationFilter('');setFuelFilter('');};
  ...
  return <Layout className="bg-[#f4f0e7]"><Header title={historyView?'Histórico de abastecimentos':editingId?'Editar abastecimento':'Novo abastecimento'} targetRoute="/fuelings" />
    <main className="min-h-0 flex-1 touch-pan-y overflow-y-auto overscroll-contain pb-[calc(2rem+env(safe-area-inset-bottom))] [-webkit-overflow-scrolling:touch]">
      ...
      {!historyView?<form onSubmit={save} className="space-y-4 px-4 pt-4">
        ...
        <button type="button" onClick={()=>void exportAll()} disabled={exporting||!filtered.length} className="flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-[#173f32] px-4 text-sm font-black text-white shadow-md disabled:opacity-50">{exporting?<Loader2 className="animate-spin" size={19}/>:<Download size={19}/>} Exportar filtrados em CSV</button><p className="mt-2 text-center text-[10px] font-semibold text-[#718078]">{isAdmin?'Você pode consultar os lançamentos de toda a equipe.':'Somente os seus lançamentos ficam disponíveis.'}</p>
        <div className="mt-4 space-y-3 rounded-2xl border border-[#d8d0c2] bg-white p-3">
          <div className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[#718078]" size={17}/><input type="search" value={search} onChange={event=>setSearch(event.target.value)} placeholder="Buscar veículo, placa, posto ou motorista" className="min-h-12 w-full rounded-xl border-2 border-[#d8d0c2] pl-10 pr-3 text-sm font-bold text-[#173f32]"/></div>
          <label className="relative block"><CalendarDays className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[#718078]" size={17}/><input aria-label="Filtrar por mês" type="month" value={month} onChange={event=>setMonth(event.target.value)} className="min-h-12 w-full rounded-xl border-2 border-[#d8d0c2] pl-10 pr-3 text-sm font-bold text-[#173f32]"/></label>
          {isAdmin&&<><label className="block text-[9px] font-black uppercase tracking-wide text-[#718078]">Técnico</label><SearchableSelect value={employeeFilter} options={employeeFilterOptions} onChange={setEmployeeFilter} placeholder="Todos os técnicos" searchPlaceholder="Buscar técnico"/></>}
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><div><label className="mb-1.5 block text-[9px] font-black uppercase tracking-wide text-[#718078]">Veículo</label><SearchableSelect value={vehicleFilter} options={historyVehicleOptions} onChange={setVehicleFilter} placeholder="Todos os veículos" searchPlaceholder="Buscar veículo ou placa"/></div><div><label className="mb-1.5 block text-[9px] font-black uppercase tracking-wide text-[#718078]">Posto</label><SearchableSelect value={stationFilter} options={historyStationOptions} onChange={setStationFilter} placeholder="Todos os postos" searchPlaceholder="Buscar posto"/></div></div>
          <label className="block text-[9px] font-black uppercase tracking-wide text-[#718078]">Combustível</label><SearchableSelect value={fuelFilter} options={fuelFilterOptions} onChange={setFuelFilter} placeholder="Todos os combustíveis" searchPlaceholder="Buscar combustível"/>
          {hasFilters&&<button type="button" onClick={clearFilters} className="flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#eef2e9] text-xs font-black text-[#315f45]"><FilterX size={16}/>Limpar filtros</button>}
        </div>
        ...
        {loading?<div className="flex min-h-40 items-center justify-center"><Loader2 className="animate-spin text-[#3f7457]" size={28}/></div>:!filtered.length?<div className="rounded-[22px] border border-dashed border-[#c9c1b4] bg-white/60 px-6 py-12 text-center"><Fuel className="mx-auto text-[#839087]" size={32}/><h3 className="campo-display mt-3 text-xl text-[#173f32]">Nenhum abastecimento</h3><p className="mt-1 text-xs font-semibold text-[#718078]">Registre o primeiro lançamento ou altere os filtros.</p></div>:<div className="space-y-3">{filtered.map(item=><article key={item.id} className="overflow-hidden rounded-[20px] border border-[#d8d0c2] bg-white shadow-sm">
          <div className="flex items-start gap-3 p-4"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#f4e7bd] text-[#805e00]"><Fuel size={21}/></span><div className="min-w-0 flex-1"><h3 className="truncate text-sm font-black text-[#173f32]">{item.vehicle}</h3><p className="mt-1 text-[10px] font-bold text-[#718078]">{item.vehiclePlate&&<span className="mr-2 font-black tracking-wide text-[#315f45]">{item.vehiclePlate}</span>}{item.date.split('-').reverse().join('/')} · {item.time} · {fuelLabel(item.fuelType)}</p></div><strong className="shrink-0 text-sm text-[#173f32]">{money.format(item.totalValue)}</strong></div>
          <div className="grid grid-cols-3 divide-x divide-[#e7e2d8] border-y border-[#e7e2d8] bg-[#f8f6f0] py-2.5"><div className="px-2 text-center"><small className="block text-[8px] font-black uppercase text-[#829087]">Litros</small><b className="mt-1 block text-xs text-[#173f32]">{decimal(item.liters,2)}</b></div><div className="px-2 text-center"><small className="block text-[8px] font-black uppercase text-[#829087]">R$/litro</small><b className="mt-1 block text-xs text-[#173f32]">{money.format(item.pricePerLiter)}</b></div><div className="px-2 text-center"><small className="block text-[8px] font-black uppercase text-[#829087]">Hodômetro</small><b className="mt-1 block text-xs text-[#173f32]">{decimal(item.odometer)} km</b></div></div>
          <div className="space-y-1.5 px-4 py-3 text-[10px] font-bold text-[#627168]"><p className="truncate"><UserRound className="mr-1 inline" size={13}/>{item.driverName}</p>{item.station&&<p className="truncate"><MapPin className="mr-1 inline" size={13}/>{item.station}</p>}<p>Tanque completo: <strong className="text-[#315f45]">{item.fullTank?'Sim':'Não'}</strong></p>{item.notes&&<p className="rounded-lg bg-[#f5f2eb] p-2 font-semibold leading-4">{item.notes}</p>}</div>
          <div className="flex justify-end gap-2 border-t border-[#ece7dd] px-4 py-3"><button type="button" onClick={()=>edit(item)} className="flex min-h-10 items-center gap-1.5 rounded-lg bg-[#e5ecdf] px-3 text-xs font-black text-[#315f45]" aria-label={`Editar abastecimento de ${item.vehicle}`}><Edit3 size={16}/>Editar</button><button type="button" onClick={()=>void remove(item)} className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#f4e1d9] text-[#8a3f31]" aria-label={`Excluir abastecimento de ${item.vehicle}`}><Trash2 size={16}/></button></div>
        </article>)}</div>}
      </section>}
    </main>

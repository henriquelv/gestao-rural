...
try {
  ...
  await click('Salvar abastecimento');await until(`location.hash==='#/fuelings/history'&&document.body.textContent.includes('Exportar filtrados em CSV')&&document.body.textContent.includes('Corolla QA')`,'fueling saved');
  ...
  await send('Page.navigate',{url:`${base}/#/fuelings/history`});await until(`document.body.textContent.includes('Exportar filtrados em CSV')&&document.body.textContent.includes('Corolla QA')`,'history after edit check');
  ...
  await send('Browser.setDownloadBehavior',{behavior:'allow',downloadPath:artifacts});await click('Exportar filtrados em CSV');
  ...
  await send('Page.navigate',{url:`${base}/#/fuelings/history`});await until(`document.body.textContent.includes('Exportar filtrados em CSV')`,'B history');assert.equal(await evaluate(`document.body.textContent.includes('Corolla QA')`),false);
  ...
  await send('Page.reload',{ignoreCache:true});await until(`document.body.textContent.includes('Todos os técnicos')&&document.body.textContent.includes('Corolla QA')`,'admin history');
  ...
  await evaluate(`[...document.querySelectorAll('[role="dialog"] button')].find(b=>b.textContent.includes('Técnico B')).click()`);await until(`document.body.textContent.includes('Nenhum abastecimento')`,'admin filter applied');

...
try {
  ...
  await send('Page.navigate',{url:`${base}/#/fuelings/history`});await until(`document.body.textContent.includes('Exportar CSV')`,'B history');assert.equal(await evaluate(`document.body.textContent.includes('Corolla QA')`),false);
  ...
  await send('Page.reload',{ignoreCache:true});await until(`document.body.textContent.includes('Mais filtros')&&document.body.textContent.includes('Corolla QA')`,'admin history');await click('Mais filtros');await until(`document.body.textContent.includes('Todos os técnicos')`,'admin filters');
  ...
  await evaluate(`[...document.querySelectorAll('[role="dialog"] button')].find(b=>b.textContent.includes('Técnico B')).click()`);await until(`document.body.textContent.includes('Nenhum resultado')`,'admin filter applied');

...
try{
  ...
  await waitFor(text('Abastecimentos'),'home');await check('Home possui seis módulos para o administrador',`document.querySelectorAll('section[aria-label="Módulos disponíveis"] > button').length===6`);await check('Indicadores visíveis',text('Indicadores da Fazenda'));await check('Consolidado não ocupa módulo separado',`!${text('Gestão Campo Legado')}`);
  await evaluate(`location.hash='#/farm-indicators'`);await waitFor(text('Indicadores da Fazenda'),'indicadores');await evaluate(`document.querySelector('button[aria-haspopup="dialog"]')?.click()`);await waitFor(text('Gestão Campo Legado'),'empresa consolidada');await check('Gestão Campo Legado aparece dentro do seletor de fazenda',text('visão administrativa'));
